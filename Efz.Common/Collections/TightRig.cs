...
namespace Efz.Collections {
  ...
  /// The collections inner array will match the number of items closely.
  /// </summary>
  public class TightRig<T> : IEnumerable<T>, IDisposable {
    ...
    /// <summary>
    /// Initializes a new TightRig with default starting size 8.
    /// </summary>
    public TightRig() {
      Capacity = 1;
    ...
    public TightRig(T[] array, int index = 0, int count = -1) {
      if(array.Length == 0) {
    ...
    /// Returns 'default(T)' if the collection contains no items.
    /// </summary>
    public T Random() {
      if(Count == 0) return default(T);
      return Array[Randomize.Range(0, Count)];
    }
    ...
    public override string ToString() {
      ...
      return string.Format("\n[TightRig Count={0}, Capacity={1}, Items=\n[\n    {2}\n]]\n", Count, Capacity, sb);
    }
