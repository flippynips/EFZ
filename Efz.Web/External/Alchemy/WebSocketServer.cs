...
namespace Alchemy {
  ...
  /// </summary>
  public class WebSocketServer : HttpServer, IDisposable {
    ...
    /// <summary>
    /// Lock for context collections.
    /// </summary>
    private static Efz.Threading.Lock _lockContexts;
    /// <summary>
    /// Lock for connection collections.
    /// </summary>
    private static Efz.Threading.Lock _lockConnections;

    static WebSocketServer() {
      _lockContexts = new Efz.Threading.Lock();
      _lockConnections = new Efz.Threading.Lock();
    ...
    private static void HandleClientThread() {
      while (!cancellation.Token.IsCancellationRequested) {
        ...
        _lockContexts.Release();

        _lockConnections.Take();
      ...
      try {
        ...
        if (_context.ReceiveReady.Wait(TimeOut, cancellation.Token)) {
          ...
          try {
            ...
            Efz.Log.Error("Error while receiving socket data.", ex);
            _context.Disconnect();
          }

        } else {
          //logger.Error("Timeout waiting for ReceiveReady");
          Efz.Log.Warning("Timeout waiting for ReceiveReady.");
          _context.Disconnect();
        }
